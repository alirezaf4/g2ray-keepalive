...
- The devcontainer now starts a small keepalive task automatically when the Codespace launches.
- The VLESS link is printed in the terminal after startup, so you do not need to manually run the service.
- If the Codespace still idles out, make sure the public port 443 is forwarded and the browser/terminal is connected.
